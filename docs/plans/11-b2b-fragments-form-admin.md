...
> 권장: **미포함** — form 자체는 폼 컴포넌트 + actions. 다이얼로그 wrap 은 admin fragment 가 처리.
>
> **Q-B. admin fragment 가 만드는 페이지의 의도된 라우트 위치?**
> 권장: `src/app/(authed)/admin/{{nameKebab}}/` — 명확한 admin 그룹.
>
> **Q-C. fragment 간 의존성 — 강제 vs 권고?**
> 권장: **권고만** (description 에 명시) — 강제는 검증/우회 메커니즘이 복잡해짐. 사용자가 `trellis add model X && trellis add service XService && trellis add admin X` 순서를 알아서.
> 대안: meta.json 에 `requiresFragments: ["model"]` 강제 — P12 또는 P13 에서 재검토.
